...
import { getFilterParams, UserFilter } from './user-filter-resource';
...
export class UserService {
  ...
  constructor(private http: AppHttClient, private auth: AuthService) {
    this.API_URL = `${environment.apiUrl}/user`;
  }

  async create(user: User): Promise<User> {
    return this.http.post<User>(this.API_URL, user).toPromise();
  }

  async readAll(): Promise<User[]> {
    return this.http.get<User[]>(`${this.API_URL}/list`).toPromise();
  }
